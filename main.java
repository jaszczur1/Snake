...
public class main extends Canvas implements KeyListener {
	...
	void gra() {
		...
		try {
			...
			if( board[m.list.get(0).y][m.list.get(0).x] == 2 ) {
				
				Point p = new Point();
			    p.x = m.list.get(0).x;
			    p.y = m.list.get(0).y;
			    p.direction =  m.list.get(0).direction;
			    list.add(0, p);
				System.err.println("drukuj");
		}
		  ...
		  if( m.list.get(0).x*10 == width-10){czyKoniecGry= true;}
		  if( m.list.get(0).x*10 == 0){czyKoniecGry= true;}
		 
		  
		  if( m.list.get(0).y*10 == height-30){czyKoniecGry= true;}
		  
		  if( m.list.get(0).y*10 == 0){czyKoniecGry= true;}
